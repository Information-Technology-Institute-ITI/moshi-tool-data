...
def migrate_workspace_command(
    ...
    backup_directory: Path | None = typer.Option(None, "--backup-directory"),
) -> None:
    """Dry-run, apply, or verify the two-service workspace migration."""
    from moshi_data_pipeline.studio.workspace_migration import migrate_workspace

    if apply and verify:
        console.print("[red]ERROR:[/red] Choose --apply or --verify, not both")
        raise typer.Exit(2)
    try:
        ...
        if verify and not report["verification"]["valid"]:
            raise typer.Exit(2)
    except (OSError, RuntimeError, ValueError) as exc:
        console.print(f"[red]ERROR:[/red] {exc}")
        raise typer.Exit(1) from exc
